...
flowchart LR
  ...
  subgraph browser["Container: Chrome / Edge Extension (Manifest V3)"]
    popup["Popup UI<br/>模式、设置、ML 控制台"]
    tracker["Tab Tracker<br/>焦点、停留时间、交互"]
    category["Category Engine<br/>域名映射 + 本地信号"]
    hygiene["Hygiene Orchestrator<br/>Check、AI Clean、测试标记"]
    ipc["Native Messaging Client<br/>Protocol v2 JSON"]
    closedLog[("Closed Tab Log<br/>恢复记录")]
    settings[("Local Settings<br/>阈值、日历、白名单")]
  end
  ...
  subgraph native["Container: macOS Swift Companion"]
    ...
    artifacts[("Application Support<br/>事件、lookup、模型指标")]
  end

  coreml[["Apple Core ML Runtime<br/>ANE / GPU / CPU scheduler"]]
  chrome[["Chrome / Edge APIs<br/>tabs, idle, sessions, system"]]

  user -->|"打开 Popup / 修改设置"| popup
  chrome -->|"标签页、闲置、会话、MEM/CPU 信号"| tracker
  popup -->|"手动 Check / AI Clean"| hygiene
  tracker --> category --> hygiene
  settings --> hygiene
  hygiene -->|"关闭、标记、恢复"| chrome
  hygiene --> closedLog
  tracker -->|"活动样本"| ipc
  category -->|"模糊页面元数据"| ipc
  ipc <-->|"Native Messaging stdio"| collector
  ipc <-->|"预测、健康状态、分类"| predictor
  collector --> artifacts
  artifacts --> predictor
  classifier --> predictor
  predictor --> coreml
  coreml -->|"闲置置信度曲线"| predictor
