...
追踪器只是信号采集层，不直接决定是否关闭网页。它为每个标签页记录四类事实：什么时候进入前台、什么时候离开前台、累计前台停留多久、发生过多少交互。关闭判断只看背景时钟（`now - lastBackgroundedAt`）；前台停留和交互次数只作为学习信号，用来估计某类网页的重要性。当前 active、pinned、audible 标签页在任何自动关闭前都会被保护。
