...
- Fixed (2026-05-01): Closure Learning UI in popup redesigned from chaotic `flex-wrap` rows to clean stacked cards (`cl-card`). Each category gets a header line (name + counts + recommendation) and a detail line (⏱ dwell / 💤 bg-age / 📏 default) with emoji prefixes for scannability. Removed the unused legend section.
